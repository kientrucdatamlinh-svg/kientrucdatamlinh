...
def update_live_overrides():
    ...
    print('Updated live_overrides.json with NEW_TIMESTAMP successfully.')
...
def update_default_data():
    ...
    code = re.sub(r'const DEFAULT_DATA_TIMESTAMP = \d+;', f'const DEFAULT_DATA_TIMESTAMP = {NEW_TIMESTAMP};', code)
    ...
    if m:
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(code)
        print('Updated default_data.js with NEW_TIMESTAMP successfully.')

# 3. Update tin-tuc.html
def update_tin_tuc():
    path = r'c:\Users\Mon\Desktop\kientrucdatamlinh\tin-tuc.html'
    with open(path, 'r', encoding='utf-8') as f:
        html = f.read()

    # Update featured card spotlight image and excerpt
    html = re.sub(
        r'(<div class="featured-card"[^>]*>.*?<div class="featured-img-wrap">\s*<img src=")[^"]+(")',
        r'\g<1>' + updated_article_1['image'] + r'\g<2>',
        html,
        flags=re.DOTALL
    )
    html = re.sub(
        r'(<p class="featured-excerpt">\s*).*?(\s*</p>)',
        r'\g<1>' + updated_article_1['excerpt'] + r'\g<2>',
        html,
        flags=re.DOTALL
    )

    # Update ARTICLES_DATA inline
    pattern = re.compile(r'(\s*\{\s*id:\s*1,\s*title:.*?\n\s*\},?)', re.DOTALL)
    ...
    # Update loadLiveTinTucData function to force overwrite localStorage when live_overrides.json is loaded
    old_func_pattern = re.compile(r'// --- Ưu tiên live_overrides\.json > localStorage > DEFAULT_ARTICLES inline ---\s*\(function loadLiveTinTucData\(\)\s*\{.*?\}\)\(\);', re.DOTALL)
    
    new_func = """// --- Ưu tiên live_overrides.json > localStorage > DEFAULT_ARTICLES inline ---
        (function loadLiveTinTucData() {
            function applyArticles(articlesList, ts) {
                if (Array.isArray(articlesList) && articlesList.length > 0) {
                    ARTICLES_DATA = articlesList;
                    if (typeof localStorage !== 'undefined') {
                        try {
                            localStorage.setItem('ktdtl_articles', JSON.stringify(articlesList));
                            localStorage.setItem('ktdtl_last_saved_ts', (ts || Date.now()).toString());
                        } catch(e) {}
                    }
                    renderArticles();
                }
            ...
            fetch('/live_overrides.json?t=' + Date.now(), { cache: 'no-store' })
                .then(function(r) { return r.ok ? r.json() : null; })
                .then(function(data) {
                    if (data && Array.isArray(data.articles) && data.articles.length > 0) {
                        applyArticles(data.articles, data.timestamp);
                    } else if (typeof localStorage !== 'undefined' && localStorage.getItem('ktdtl_articles')) {
                        try { ARTICLES_DATA = JSON.parse(localStorage.getItem('ktdtl_articles')); } catch(e) {}
                        renderArticles();
                    } else {
                        renderArticles();
                    }
                ...
                .catch(function(e) {
                    console.warn('live_overrides fetch error', e);
                    if (typeof localStorage !== 'undefined' && localStorage.getItem('ktdtl_articles')) {
                        try { ARTICLES_DATA = JSON.parse(localStorage.getItem('ktdtl_articles')); } catch(e) {}
                    }
                    renderArticles();
                });
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(html)
    print('Updated tin-tuc.html with cache buster logic successfully.')
